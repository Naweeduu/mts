...
if(window.innerWidth <= 992){
  if(document.querySelector('.del__body')) {
    ...
    arrImg[0].before(textImg.textContent = 'Multimedia launcher')
    arrImg[1].before(textImg.textContent = 'Multimedia')
    arrImg[2].before(textImg.textContent = 'Multimedia Video dash cam')

    arrImg[3].before(textImg.textContent = 'Multimedia launcher')
    arrImg[4].before(textImg.textContent = 'Multimedia')
    arrImg[5].before(textImg.textContent = 'Видеорегистратор')

    arrImg[6].before(textImg.textContent = 'Multimedia launcher')
    arrImg[7].before(textImg.textContent = 'Мультимедийная система')
    arrImg[8].before(textImg.textContent = 'Multimedia Video dash cam')
  }
...
if(window.innerWidth <= 480){
  ...
  btnOpen.addEventListener('click', function() {
      ...
      if(btnOpen.classList.contains('active') == true) {
        btnOpen.innerHTML = '&nbsp&nbsp&nbsp&nbspHide&nbsp&nbsp&nbsp&nbsp';
      }
      if(btnOpen.classList.contains('active') == false) {
        btnOpen.innerHTML = 'Read more';
      }
  })
...
document.querySelector('#showMore').addEventListener('click', function (e){
    ...
    if(document.querySelector('#text').classList.contains('active')){
      document.querySelector('#showMore').innerHTML ='Read more';
    } else {
      document.querySelector('#showMore').innerHTML ='&nbsp&nbsp&nbsp&nbspHide&nbsp&nbsp&nbsp&nbsp';
    }
